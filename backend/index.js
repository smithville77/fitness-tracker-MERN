...
app.get("/runDisplayPage", async (req, res) => {
  try {
    ...
    if (accessToken) {
      ...
      const secondSetActivitiesResponse = await axios.get(
        secondSetActivitiesUrl,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
      ...
      const firstSetRunActivities =
        allActivitiesResponse.data.activities.filter(
          (activity) => activity.activityTypeId === 90009
        );

      const secondSetRunActivities =
        secondSetActivitiesResponse.data.activities.filter(
          (activity) => activity.activityTypeId === 90009
        );

      const runActivities = firstSetRunActivities.concat(
        secondSetRunActivities
      );
